...
def main():
    ...
    try:
        total_rows = dataset.info.splits["full"].num_examples
    except Exception:
        total_rows = None
    rows = []
    last_pct = -1
    ...
    # Exclude columns with unhashable types (like lists) for hashing
    df_for_hash = df.drop(columns=[col for col in df.columns if df[col].apply(lambda x: isinstance(x, list)).any()])
    h = hashlib.sha256(pd.util.hash_pandas_object(df_for_hash, index=True).values).hexdigest()
